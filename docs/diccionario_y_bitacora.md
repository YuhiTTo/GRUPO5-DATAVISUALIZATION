...
* **Modelado Relacional en Esquema Estrella (Entrega 4):** Se estructuró la base limpia en una Tabla de Hechos (`fact_hogares.csv`) y Dimensiones (`dim_segmento.csv`), reduciendo redundancia y garantizando compatibilidad de tipado estricto (Enteros para IDs y String con `zfill(6)` para UBIGEO) en Tableau. Se recalculó la matemática de `_PCT` sobre la canasta monetaria estricta de 8 rubros para garantizar sumas exactas de 100%.
* **Diseño Analítico de Visualización Exploratoria (Entrega 5):** Se estableció el enfoque de "Problema Único Crítico" (Laser-Focused en la trampa del Déficit Crítico alimentario del Clúster 4). Se generó la matriz de selección oficial de 4 gráficos (Composición en Stacked Bar, Ranking Departamental en Barras Horizontales, Tendencia Estacional en Líneas y Distribución en Boxplot) y se fundamentó empíricamente el descarte técnico de 2 gráficos preliminares (Scatter crudo de 33k puntos por *Overplotting* y Treemap/Burbujas por asimetría de magnitudes).
